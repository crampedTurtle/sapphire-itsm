...
def upgrade() -> None:
    ...
    # Note: Even with create_type=False, SQLAlchemy may still try to create types
    # If types already exist from SQL scripts, this is fine - they'll be reused
    ...
    op.create_table(
        ...
        sa.Column('plan_tier', sa.Enum('tier0', 'tier1', 'tier2', name='plantier', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('role', sa.Enum('customer', 'agent', 'ops', name='identityrole', create_type=False), nullable=False),
        sa.Column('created_at', sa.DateTime(), nullable=False),
    )
    ...
    op.create_table(
        'intake_events',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('source', sa.Enum('email', 'portal', name='intakesource', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('intent', sa.Enum('sales', 'support', 'onboarding', 'billing', 'compliance', 'outage', 'unknown', name='intent', create_type=False), nullable=False),
        sa.Column('urgency', sa.Enum('low', 'normal', 'high', 'critical', name='urgency', create_type=False), nullable=False),
        sa.Column('confidence', sa.Float(), nullable=False),
        sa.Column('compliance_flag', sa.Boolean(), nullable=False),
        sa.Column('recommended_action', sa.Enum('self_service', 'create_case', 'route_sales', 'escalate_ops', 'needs_review', name='recommendedaction', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('new', 'open', 'pending_customer', 'pending_internal', 'escalated', 'resolved', 'closed', name='casestatus', create_type=False), nullable=False, index=True),
        sa.Column('priority', sa.Enum('low', 'normal', 'high', 'critical', name='casepriority', create_type=False), nullable=False, index=True),
        sa.Column('category', sa.Enum('support', 'onboarding', 'billing', 'compliance', 'outage', name='casecategory', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('sender_type', sa.Enum('customer', 'agent', 'system', name='sendertype', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('artifact_type', sa.Enum('summary', 'draft_reply', 'kb_answer', name='artifacttype', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('tenant_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('tenants.id'), nullable=False, index=True),
        sa.Column('tier', sa.Enum('tier0', 'tier1', 'tier2', name='plantier', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('case_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('cases.id'), nullable=False, index=True),
        sa.Column('event_type', sa.Enum('started', 'first_response', 'breached_first_response', 'breached_resolution', 'paused', 'resumed', name='slaeventtype', create_type=False), nullable=False),
